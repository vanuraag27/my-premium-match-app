...
export async function POST(req) {
  try {
    ...
    if (!isSessionAuthorized(req, sid)) {
...
export async function PATCH(req) {
  try {
    ...
    if (!['accept', 'reject', 'revoke_rejection'].includes(action)) {
      return NextResponse.json(
        { success: false, error: 'Action must be "accept", "reject", or "revoke_rejection".' },
        { status: 400 }
      );
    }
    ...
    // A receiver may revoke their own rejection. This does not accept the
    // request or open messaging; it simply restores the sender's ability to
    // submit a fresh message request. The original rejection remains in the
    // audit trail as a Revoked record.
    if (action === 'revoke_rejection') {
      if (request.requestStatus !== REQUEST_STATUS.REJECTED) {
        return NextResponse.json(
          { success: false, error: 'Only a rejected message request can have its rejection revoked.' },
          { status: 409 }
        );
      }

      if (String(request.receiverId) !== uid || String(request.rejectedBy || '') !== uid) {
        return NextResponse.json(
          { success: false, error: 'Only the receiver who rejected the request can revoke that rejection.' },
          { status: 403 }
        );
      }
      ...
      await db.collection('message_requests').updateOne(
        ...
        {
          $set: {
            requestStatus: REQUEST_STATUS.REVOKED,
            rejectionRevokedAt: now,
            rejectionRevokedBy: uid,
          },
        }
      );
      ...
      return NextResponse.json({
        ...
        message: 'Rejection revoked. The sender may now send a new message request.'
      });
    }
